...
1. **[`syckpt/storage.py`: Content Addressable Storage & Delta Compression](./storage_and_cas.md)**
   ...
   * **Sub-Layer Freezing:** Analyze exactly how transfer-learning zero-variance paths mimic Docker hard-linking to achieve zero-cost storage for frozen backbones.
...
2. **[`syckpt/manager.py`: Orchestration & PyTorch DDP Synchronization](./manager_and_ddp.md)**
   ...
   * **Asynchronous Multiprocessing:** Analyze exactly how Global Interpreter Lock (GIL) is bypassed via isolated OS processes to eliminate GPU I/O-bound pipeline stalling.
...
4. **[`syckpt/dataloader.py`: Slicing Iterators & Exact Resumption](./dataloader_and_resumption.md)**
   ...
   * **$O(1)$ Resumption:** Deconstructs how `StatefulRandomSampler` subclassing bypasses primitive generator `next()` skipping to enforce absolute zero-cost index resumption natively.
